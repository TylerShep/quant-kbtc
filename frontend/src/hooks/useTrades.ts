...
const cache: Record<string, TradesResponse> = {};
...
export function useTrades(page: number, perPage = 10, mode: string = 'paper') {
  ...
  const [data, setData] = useState<TradesResponse | null>(cache[key] ?? null);
  ...
  useEffect(() => {
    ...
    async function fetchActive() {
      ...
      if (!cache[otherKey]) {
        try {
          const res = await fetch(`/api/trades?page=1&per_page=${perPage}&mode=${other}`);
