"""
DEPRECATED: BTC-spot-price backtester.

Kept for reference and compatibility while the contract-price simulator
becomes the default.
"""
...
class SpotBacktester:
    ...
    def run(self, bankroll: float = 10000.0) -> dict:
        ...
        hard_stop_loss_pct = self.config.get(
        ...
        for i, candle in enumerate(self.candles):
            ...
            if position:
                ...
                worst_price = (
                    candle["low"] if position["direction"] == "long"
                    else candle["high"]
                )
                worst_pnl_pct = self._calc_pnl_pct(position, worst_price)
                position["max_adverse_excursion"] = min(
                    position.get("max_adverse_excursion", 0.0), worst_pnl_pct
                )

                if (hard_stop_loss_pct > 0.0
                        and worst_pnl_pct <= -hard_stop_loss_pct):
                    candle_for_exit = dict(candle)
                    candle_for_exit["close"] = worst_price
                    self._close_position(
                        position, candle_for_exit, "HARD_STOP_LOSS",
                        current_bankroll,
                    )
